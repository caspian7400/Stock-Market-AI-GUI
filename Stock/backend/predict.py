...
def predict_stock(symbol, period, sim, future):
    ...
    class Model(tf.Module):
        def __init__(self, learning_rate, num_layers, size, size_layer, output_size, forget_bias=0.1):
            ...
            # Replace the explicit placeholder for self.hidden_layer with a default value
            self.hidden_layer = tf.compat.v1.placeholder_with_default(
                np.zeros((1, num_layers * 2 * size_layer), dtype=np.float32),
